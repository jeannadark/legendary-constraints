...
def backtrack_search(csp: Any, graph: Any) -> Any:
    ...
    try assigning the least constraining color to it if the assignment is consistent 
    (i.e. that it has not been assigned to any other vertex) and if yes, then
    perform an arc consistency check upon which will keep recursively backtracking until all vertices have colors
    assigned to them.

    If no solution found with the current color assignment, then it is de-assigned.
